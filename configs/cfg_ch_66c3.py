...
# cfg.data_folder = f"{BASEPATH}/datamount/train_landmarks_v1/"
# cfg.train_df = f"{BASEPATH}/datamount/train_v2.csv"

cfg.name = os.path.basename(__file__).split(".")[0]
cfg.output_dir = f"/mount/asl/models/{os.path.basename(__file__).split('.')[0]}"

cfg.data_folder = f"/raid/asl-fingerspelling/train_landmarks_v3/"
cfg.train_df = f'/mount/asl/models/cfg_ch_38/train_v2_score38_oof_supp.csv'
...
with open('/mount/asl/data/character_to_prediction_index.json', "r") as f:
    char_to_num = json.load(f)

# with open(f'{BASEPATH}/datamount/character_to_prediction_index.json', "r") as f:
#     char_to_num = json.load(f)
